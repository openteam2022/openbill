<template>
	<view class="add">
		...
		<counter :selectCategory="selectCategory" :money="money"  @addOther="addOther" :addNotes="addNotes" :currentTime="currentTime" @onChange="onChange" @save="saveBill" @chooseDate="chooseDate"></counter>
...
<script>
	import {addBill,createBill,updateBill} from '@/common/sqlite.js';
	import Dates from '@/common/date.js'
	import {checkNull} from '@/common/shield.js'
	import open from '@/common/comm.js'
	import {showInfo,goTabbarUrl} from '@/common/base.js'
	...
	export default {
		...
		methods:{
			...
			saveBill(){
				if(this.userInfo.id){
					//如果记录账单金额为0,抛出失败信息
					if(!checkNull(this.money,'请输入金额'))return false;
					// 用户选择的记账日期
					const date = new Dates()
					const billTime = this.billDate || date.getTime();
					//保留记账金额2个小数点
